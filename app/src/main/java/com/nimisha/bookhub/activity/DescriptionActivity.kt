...
class DescriptionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (ConnectionManager().checkConnectivity(this@DescriptionActivity)) {
            val jsonRequest =
                object :
                    JsonObjectRequest(Method.POST, url, jsonParams, Response.Listener<JSONObject> {
                        try {
                            ...
                            if (success) {
                                ...
                                btnAddToFav.setOnClickListener {
                                    if (!DBAsyncTask(
                                            applicationContext, bookEntity, 1
                                        ).execute().get()
                                    ) {
                                        val async =
                                        ...
                                        if(result){
                                            Toast.makeText(
                                                this@DescriptionActivity,"Book added to Favorites",Toast.LENGTH_SHORT
                                            ).show()
                                        ...
                                        if (result){
                                            Toast.makeText(
                                                this@DescriptionActivity,"Book removed from Favorites",Toast.LENGTH_SHORT
                                            ).show()
    ...
    class DBAsyncTask(val context: Context, val bookEntity: BookEntity, val mode: Int) :
        ...
        override fun doInBackground(vararg p0: Void?): Boolean {
            when (mode) {
                1 -> { //check db if book is in favorite
                    val book: BookEntity? = db.bookDao().getBookById(bookEntity.book_id.toString())
                    db.close()
                    return book != null
                }

                2 -> { //save the book into db as favorite
